...
- Open the latest artifact for the gate stage
- Inspect the latest run directory when validation or runner behavior is unclear
- Ask the user for an explicit approve, reject, or rewind decision
- Use `approve`, `reject`, or `rewind`; never edit `state.json` or `engine_state.json`
